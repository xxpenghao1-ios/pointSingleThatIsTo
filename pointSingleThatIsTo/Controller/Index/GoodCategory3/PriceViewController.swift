...
extension PriceViewController{
    ...
    func httpSelectGoodCategoryList(currentPage:Int,isRefresh:Bool){
        var count=0
        Alamofire.request(.GET,URL+"queryGoodsInfoByCategoryForAndroidForStore.xhtml",parameters:["goodsCategoryId":goodsCategoryId!,"countyId":countyId!,"IPhonePenghao":520,"isDisplayFlag":2,"pageSize":10,"currentPage":currentPage,"storeId":storeId!,"order":"price"]).responseJSON{ response in
            if response.result.error != nil{
                //关闭刷新状态
                self.goodTable?.headerEndRefreshing()
                //关闭加载状态
    ...
    func httpSearchGoodList(currentPage:Int,isRefresh:Bool){
        /// 定义一个int类型的值 用于判断是否还有数据加载
        var count=0
        Alamofire.request(.GET,URL+"searchGoodsInterfaceForStore.xhtml", parameters:["searchCondition":searchName!,"countyId":countyId!,"IPhonePenghao":520,"isDisplayFlag":2,"pageSize":10,"currentPage":currentPage,"storeId":storeId!,"order":"price"]).responseJSON{ response in
            if response.result.error != nil{
                //关闭刷新状态
                self.goodTable?.headerEndRefreshing()
                //关闭加载状态
    ...
    func httpQueryStorePromotionGoodsList(currentPage:Int,isRefresh:Bool){
        /// 定义一个int类型的值 用于判断是否还有数据加载
        var count=0
        request(.GET,URL+"queryStorePromotionGoodsList.xhtml", parameters:["storeId":storeId!,"order":"price","pageSize":10,"currentPage":currentPage]).responseJSON{ response in
            if response.result.error != nil{
                //关闭刷新状态
                self.goodTable?.headerEndRefreshing()
                //关闭加载状态
