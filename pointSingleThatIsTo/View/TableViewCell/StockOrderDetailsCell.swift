...
class StockOrderDetailsCell:UITableViewCell{
    ...
    override init(style: UITableViewCellStyle, reuseIdentifier: String?) {
        ...
        //初始化左边文字
        lblLeftText=UILabel()
        lblLeftText!.frame = CGRect(x: 10, y: (contentH-20)/2, width: boundsWidth-20, height: 20)
        lblLeftText?.font=UIFont.systemFont(ofSize: 14)
    ...
    */
    func loadHaveGoods(_ entity:OrderListEntity,i:Int){
        ...
        if(goodsList.count > 0){
            ...
            //商品可不可退
            lblReturnGoodsFlag?.frame=CGRect(x: goodsPic!.frame.maxX+5, y: goodsPic!.frame.maxY-40, width: 200, height: 20)
            lblReturnGoodsFlag?.font=UIFont.systemFont(ofSize: 13)
